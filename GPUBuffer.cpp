...
//	    std::cout << i << ":" << mesh->mVertices[i].x << " " << mesh->mVertices[i].y << " " << mesh->mVertices[i].z << std::endl;
	...
	for (unsigned int t = 0; t < mesh->mNumFaces; ++t) {
		...
		std::cout << "hey" << _elementArray.back() << std::endl;
	    }
	}
    }
    for (n = 0; n < nd->mNumChildren; ++n) {
	getAllFaces(sc, nd->mChildren[n]);
	if (_elementArray.size() > 1)
		std::cout << "plopi" << _elementArray.back() << std::endl;
    }
}
...
void GRand::GPUBuffer::draw(GLenum drawStyle_) const noexcept {
    ...
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, _ebo);
    glBindBuffer(GL_ARRAY_BUFFER, _vbo);
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(decltype(_vertexArray)::value_type), (void*)0);
